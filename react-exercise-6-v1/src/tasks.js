...
function Task1(props) {
  ...
  return (
    <Box maxW="sm" marginX="auto" bgColor="blue.800" color="white" borderWidth="1px" borderRadius="lg" overflow="hidden">
...
function Task2({ properties }) {
  return (
    <SimpleGrid columns={[1, 2, 4]} gap="14">
      {properties.map((aset, index) => (
        <Task1 key={index} property={aset} />
      ))}
    </SimpleGrid>
  );
}
...
function Task1_2() {
  ...
  return (
    <div>
      <h1>{name}</h1>
      <button onClick={() => setName("Mochamad Syahrul Akbar")}>Change Name</button>
    </div>
  );
}
